...
interface ApiInterfaceWG {
    ...
    @GET("wotb/account/list/?application_id=${Constants.wgAppId}")
    suspend fun getAccountId(@Query("search") search: String) : Response<ResponseBody>

    @GET("wotb/account/info/?application_id=${Constants.wgAppId}&extra=statistics.rating") //id_drnkwtr = 123297533 id_xomyak=95102841 id_light= 38687016
    suspend fun getUsers(@Query("account_id") search: String) : Response<ResponseBody>

    @GET("wotb/clans/accountinfo/?application_id=${Constants.wgAppId}&extra=clan")
    suspend fun getClanInfo(@Query("account_id") search: String) : Response<ResponseBody>

    @GET("wotb/clans/info/?application_id=${Constants.wgAppId}&extra=members") // было 218951
    suspend fun getFullClanInfo(@Query("clan_id") search: String) : Response<ResponseBody>

    @GET("wotb/account/achievements/?application_id=${Constants.wgAppId}&fields=-max_series")
    suspend fun getAchievements(@Query("account_id") search: String) : Response<ResponseBody>

    @GET("wotb/encyclopedia/vehicles/?application_id=${Constants.wgAppId}&fields=name,nation,tier,type")
    suspend fun getAllInformationAboutVehicles() : Response<ResponseBody>

    @GET("wotb/tanks/stats/?application_id=${Constants.wgAppId}")
